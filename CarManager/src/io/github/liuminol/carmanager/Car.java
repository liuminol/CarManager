...
public abstract class Car implements Serviceable {
	...
	public void addDistance(int additionalDisatance) {
		...
		try {
			if (additionalDisatance < 0) {
				throw new Exception();
			}
			distance += additionalDisatance;
			distanceOnService += additionalDisatance;
		} catch (Exception e) {
	...
	public void addDistance(double addictionalDistance) {
		double sum = Math.round((double) distance + addictionalDistance);
		distance = (int) sum;
		distanceOnService = (int) sum;
	}
